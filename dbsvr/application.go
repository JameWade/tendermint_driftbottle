...
import (
	...
	"github.com/tendermint/tendermint/abci/example/driftbottle"
	"github.com/tendermint/tendermint/abci/example/driftbottle/types"
...
func NewdriftBottleApplication() *driftBottleApplication {
	...
	appBytes, err := db.Get([]byte(appKEY))
...
func (app *driftBottleApplication) CheckTx(req tmtypes.RequestCheckTx) (rsp tmtypes.ResponseCheckTx) {
	...
	if ok {
		...
		if ok {
			btBytes, err := app.db.Get(msg.BottleID.Bytes())
			if err == nil {
				return
			}
			if btBytes == nil {
...
func (app *driftBottleApplication) DeliverTx(req tmtypes.RequestDeliverTx) (rsp tmtypes.ResponseDeliverTx) {
	...
	if ok {
		if bottle.Replier == [32]byte{} {
			...
			//将新瓶子放到池子里，等待后续打捞
			bottlePool, err := app.db.Get(bottlePoolKey)
			if err == nil {
				return
			}
			if bottlePool == nil {
...
func (app *driftBottleApplication) Query(req tmtypes.RequestQuery) (rsp tmtypes.ResponseQuery) {
	...
	if submatches == nil {
		...
		idx := rand.Intn(psize)
		btBytes, err := app.db.Get(bottlePool[idx*idlen : idx*idlen+idlen])
		if err == nil {
			return
		}
		rsp.Value = btBytes
		...
		if matchmap["bid"] != "" {
			...
			if matchmap["msgs"] != "" {
				...
				rsp.Value, err = app.db.Get(msgKey)
				if err == nil {
					return
				}
				return
			...
			btBytes, err := app.db.Get(btKey.Bytes())
			if err == nil {
				return
			}
			rsp.Value = btBytes
